	...
	namespace Tests
	{
		...
		void	TestEndLog(const char* csFunction, const uint32 iTestCount, const uint32 iSuccessful)
		{
			if (iTestCount == iSuccessful)
				FT_COUT << "Test: Succès: " << csFunction << " (" << iSuccessful << '/' << iTestCount << ')' << std::endl;
			else
				FT_CERR << "Test: ECHEC: " << csFunction << " | " << iTestCount - iSuccessful << " tests échoués (" << iSuccessful << '/' << iTestCount << ')' << std::endl;
		}
	}
}
